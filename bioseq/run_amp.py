        ...
        if args.gen_data_sample_per_step > 0 and dataset is not None:
            ...
            for i in range(len(x)):
                ...
                for c, a in zip(x[i] + self.eos_char, self.tokenizer.process([x[i] + self.eos_char])[0]):
                    ...
                    traj_rewards[i+m].append(0 if c != self.eos_char else self.l2r(y[i], 0))
...
def sample_offline(args, rollout_worker, generator, current_dataset, oracle, sample_size):
    print("Generating new offline samples")
    samples = []
    while len(samples) < sample_size:
        rollout_artifacts = rollout_worker.execute_train_episode_batch(generator, it=0, use_rand_policy = False)
        states = rollout_artifacts["trajectories"]["states"]
        samples.extend(states)
        print('now len is',len(samples))
        if not len(samples)%1000:
            with open('new_offline_data.pkl','wb') as f:
                pickle.dump(samples,f)
    with open('new_offline_data.pkl','wb') as f:
        pickle.dump(samples,f)
    print('save offline done.')
...
def train(args, oracle, dataset):
    ...
    for round in range(args.num_rounds):
        ...
        with open(f'results/data_online.pkl','wb') as f:
            pickle.dump(batch,f)
            print('data saved')
        dataset.add(batch)
        ...
        proxy.update(dataset)
        args.logger.save(args.save_path, args)
    sample_offline(args, rollout_worker, generator, dataset, oracle,20000)
