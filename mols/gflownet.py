...
parser.add_argument("--save_path", default='online_results/weak30/')
parser.add_argument("--proxy_path", default='./data/pretrained_proxy')
parser.add_argument("--weak_proxy_path", default='./results_new_proxy30/_1')
...
parser.add_argument("--obj", default='fm', type=str)
...
parser.add_argument("--do_weak", default=False, action='store_true')
...
class Dataset:
    ...
    def set_sampling_model(self, model, proxy_reward, real_proxy_reward, sample_prob=0.5):
        self.sampling_model = model
        self.sampling_model_prob = sample_prob
        self.proxy_reward = proxy_reward
        self.real_proxy_reward = real_proxy_reward
    ...
    def _get_sample_model(self):
        ...
        self.real_r_mols.append((real_r, m, trajectory_stats, inflow))
    ...
    def _get_reward(self, m):
        ...
        return self.r2r(normscore=self.proxy_reward(m)),self.r2r(normscore=self.real_proxy_reward(m))
    ...
    def evaluate(self, epoch):
        avg_topk_rs, avg_topk_tanimoto, num_modes_above_7_5, num_modes_above_8_0, \
            num_mols_above_7_5, num_mols_above_8_0 = eval_mols(self.real_r_mols,
        ...
        avg_topk_rs_recent, avg_topk_tanimoto_recent, num_modes_above_7_5_recent, num_modes_above_8_0_recent, \
            num_mols_above_7_5_recent, num_mols_above_8_0_recent = eval_mols(self.real_r_mols[-50000:],
                reward_norm=args.reward_norm, reward_exp=args.reward_exp, algo="gfn")
        print(f"state_visited={len(self.real_r_mols)};"
...
def make_model(args, mdp, out_per_mol=1,obj='fm'):
...
class Proxy:
    def __init__(self, args, bpath, device, weak=False):
        if not weak:
            ...
            eargs = pickle.load(gzip.open(f'{args.weak_proxy_path}/info.pkl.gz'))['args']
            params = pickle.load(gzip.open(f'{args.weak_proxy_path}/best_params.pkl.gz'))
        self.mdp = MolMDPExtended(bpath)
...
def train_model_with_proxy(args, model, proxy, weak_proxy, dataset, num_steps=None, do_save=True, do_weak=False):
    ...
    if do_save:
        exp_dir = f'{args.save_path}/{args.array}_{args.run}_{args.obj}/'
        os.makedirs(exp_dir, exist_ok=True)


    dataset.set_sampling_model(model, proxy if not do_weak else weak_proxy, proxy, sample_prob=args.sample_prob)

    def save_stuff():
        pickle.dump([i.data.cpu().numpy() for i in model.parameters()],
                    gzip.open(f'{exp_dir}/params.pkl.gz', 'wb'))

        pickle.dump(dataset.real_r_mols,
    ...
    for i in range(num_steps):
        ...
        if not i%print_interval:
            dataset.evaluate(i)
    for i in range(num_steps,1000000):
...
def main(args):
    ...
    model = make_model(args, mdp, obj=args.obj)
    model.to(args.floatX)
    model.to(device)

    proxy = Proxy(args, bpath, device)
    weak_proxy = Proxy(args, bpath, device, True)

    train_model_with_proxy(args, model, proxy, weak_proxy, dataset, do_save=True, do_weak=args.do_weak)
    print('Done.')
