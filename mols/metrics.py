...
class BemisMurckoModes:
    def __init__(self, reward_exp, reward_norm, reward_thr=8):
        self.reward_exp = reward_exp
        self.reward_norm = reward_norm
        self.reward_thr = reward_thr
        self.scaffolds = set()  # 用于存储唯一的 scaffold
        self.max_reward = -1000
    ...
    def __call__(self, batch):
        ...
        # 对于每个候选分子，提取 scaffold 并检测是否是新的 scaffold
        for mol in candidates:
            scaffold = MurckoScaffold.GetScaffoldForMol(mol.mol)
            scaffold_smiles = Chem.MolToSmiles(scaffold)
            if scaffold_smiles not in self.scaffolds:
                self.scaffolds.add(scaffold_smiles)
        
        return self.max_reward, len(self.scaffolds)
