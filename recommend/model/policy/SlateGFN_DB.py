...
class SlateGFN_DB(BaseOnlinePolicy):
    ...
    def generate_action(self, user_state, feed_dict):
        ...
        #（B, K）
        support_sum_p = torch.zeros(B, slate_size).to(self.device)
        ...
        for i in range(slate_size):
            ...
            if is_train or torch.is_tensor(parent_slate):
                ...
                support_P = torch.gather(prob,1,action_at_and_after_i)
                # sum_P = torch.sum(support_P,dim=1)
                support_sum_p[:,i]= torch.sum(support_P,dim=1)
    ...
    def get_loss(self, feed_dict, out_dict):
        ...
        * terminal: ( log(flow(s[t])) - log(reward(s[t])) )^2
        ...
        # scalar
        # seems no pow(2) is better
        # DB_loss = torch.mean((forward_part - backward_part).pow(2)) + torch.mean(self.alpha*(parent_flow - support_P*parent_flow).pow(2))
        # out_diff = parent_flow - support_P*parent_flow
        # DB_loss = torch.mean((forward_part - backward_part).pow(2)) + torch.mean(self.alpha*F.smooth_l1_loss(out_diff,torch.zeros_like(out_diff), reduction="none"))
        DB_loss = torch.mean((forward_part - backward_part).pow(2)) + .0*torch.mean(parent_flow - support_P*parent_flow)
        # DB_loss = torch.mean((forward_part - backward_part).pow(2)) + torch.mean(self.alpha*(parent_flow-current_flow)) #no good
        # if self.alpha>self.alpha_decrease:self.alpha-=self.alpha_decrease
