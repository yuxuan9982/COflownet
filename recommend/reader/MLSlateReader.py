...
class MLSlateReader(MLSeqReader):
    ...
    def __getitem__(self, idx):
        ...
        F_rowIDs = [rid for rid in self.user_history[user_id] if rid >= row_id][6:self.max_hist_seq_len+6]
        # F = self.log_data.iloc[F_rowIDs]
        # future_ids = np.array([self.item_id_vocab[iid] for iid in F['movie_id']])
        # record['future_ids'] = future_ids
        future, future_length, future_meta, future_response = self.get_user_history(F_rowIDs)
        record['future'] = np.array(future)
        record['future_length'] = np.array(future_length)
